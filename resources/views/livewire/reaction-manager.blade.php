    ...
    <div class="flex w-full justify-between gap-x-4 dark:!text-white">
        <div
            @class([
                ...
                "border border-gray-200 bg-white dark:border-slate-700 dark:bg-slate-800" => Helpers::isGithubTheme(),
                "rounded-lg border-none bg-transparent dark:border-slate-700 dark:bg-slate-800" => Helpers::isModernTheme(),
            ])
        >
                ...
                @if ($key === "like")
                    <div
                        ...
                        @class([
                            ...
                            "!rounded-[1000px] rounded-lg !px-2 !py-1 dark:!bg-slate-900 dark:hover:!bg-slate-600" => Helpers::isModernTheme(),
                        ])
                        @style([
                            "background: " . config("commenter.bg_primary_color") =>
                                Helpers::isGithubTheme() || Helpers::isModernTheme(),
                        ])
                    >
                        <div
                            ...
                            @if ($authMode)
                                ...
                                ="
                                                                ...
                                                                "
                            @endif
                            class="flex min-w-[2.2rem] cursor-pointer items-center justify-between"
                            title="like"
                        >
                            <div x-show="!isLiked">
            ...
            @if ($enableReply)
                <div
                    ...
                    @reply-discarded.window="
                     ...
                     "
                    @reply-created-
                    {{ $message->getKey() }}.window="
                        if ($event.detail.messageId === @js($message->getKey())) {
                            setTimeout(() => {showReplyForm = !showReplyForm}, 2000)
                        }
                     "
                    @class([
                        "rounded px-1 dark:border-slate-700 dark:!bg-slate-800",
                        "!rounded-[1000px] bg-transparent !px-2 !py-1" => Helpers::isModernTheme(),
                        "hover:!bg-[" . config("commenter.hover_color") . "]" =>
                            Helpers::isGithubTheme() || Helpers::isModernTheme(),
                    ])
                    @style([
            ...
            @if ($shouldEnableShareButton)
                <div
                    ...
                    @class([
                        "relative gap-2 rounded px-1 dark:border-slate-700 dark:!bg-slate-800",
                        "!rounded-[1000px] bg-transparent !px-2 !py-1" => Helpers::isModernTheme(),
                        "hover:!bg-[" . config("commenter.hover_color") . "]" =>
                            Helpers::isGithubTheme() || Helpers::isModernTheme(),
                    ])
                ...
                >
                    <div x-data="copyToClipboard" class="relative">
                        ...
                        >
                            ...
                            <span x-show="isCopied" class="absolute start-[calc(100%_+_1rem)] text-nowrap">
                                Link Copied!
                            </span>
                        </x-commenter::link>
        ...
        <div
            @class([
                ...
                "border border-gray-200 bg-white dark:border-slate-700 dark:!bg-slate-800" => Helpers::isGithubTheme(),
                "!max-w-[11rem] rounded-lg bg-transparent sm:!max-w-40 md:!max-w-72 dark:border-slate-700 dark:!bg-slate-800" => Helpers::isModernTheme(),
            ])
        >
                ...
                <div class="flex py-4 dark:bg-black">
                    <div class="space-y-2 border-r-2 border-gray-200 dark:border-slate-900">
                        <div class="mb-4 border-b-2 border-gray-200 p-4 dark:border-slate-900">
                            <span
                                class="bg-gray-300 px-4 py-2 font-bold dark:bg-slate-600"
                                @style([
                        ...
                        @foreach (config("commenter.reactions") as $key => $reaction)
                            <div
                                @if ($reactions[$key]["count"] > 0)
                                    ...
                                    ="type = '{{ $key }}'"
                                    @class([
                                        "relative cursor-pointer p-4 hover:bg-gray-300 hover:dark:bg-slate-700",
                                        "hover:!bg-[" . config("commenter.hover_color") . "]",
                                    ])
                                @endif
